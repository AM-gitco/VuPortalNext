...
import { loginSchema, type LoginData } from "@/lib/db/schema"; // Updated import
import { loginAction } from "@/app/actions/auth"; // Import action
...
export function LoginForm({ onSwitchToSignup, onSwitchToForgotPassword, onSwitchToOTP }: LoginFormProps) {
  ...
  const onSubmit = (data: LoginData) => {
    startTransition(async () => {
      ...
      try {
        // We handle the result manually because if redirect happens, it might throw, 
        // but loginAction handles the redirect internally which Next.js caches. 
        // However, reading the result if success is tricky if it redirects.
        // Actually, let's catch the error if it is NOT a redirect error.
        ...
        console.error("Login Error detected in client:", error);
        // Next.js Redirect throws an error, so we ignore it if it successfully redirected (which unmounts this component usually)
      }
    });
  };

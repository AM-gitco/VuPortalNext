  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-foreground" data-testid="text-admin-title">Admin Panel</h1>
            ...
            <CardContent>
              {loadingUsers ? (
                ...
                <div className="space-y-3">
                  {users.map((u: any) => (
                    <div key={u.id} className="flex items-center justify-between p-4 bg-muted/30 dark:bg-muted/10 rounded-lg" data-testid={`row-user-${u.id}`}>
                      <div className="flex items-center gap-3">
              ...
              {loadingUploads ? (
                ...
                <div className="space-y-3">
                  {uploads.map((upload: any) => (
                    <div key={upload.id} className="flex items-center justify-between p-4 bg-muted/30 dark:bg-muted/10 rounded-lg" data-testid={`row-upload-${upload.id}`}>
                      <div>
                        <p className="font-medium text-foreground">{upload.title}</p>
                        <p className="text-sm text-muted-foreground">{upload.subject} - {upload.uploadType}</p>
                      </div>
                      <div className="flex items-center gap-2">
              ...
              {loadingDiscussions ? (
                ...
                <div className="space-y-3">
                  {discussions.map((discussion: any) => (
                    <div key={discussion.id} className="flex items-center justify-between p-4 bg-muted/30 dark:bg-muted/10 rounded-lg" data-testid={`row-discussion-${discussion.id}`}>
                      <div className="flex-1 min-w-0">
                        <p className="font-medium text-foreground truncate">{discussion.content}</p>
                        <p className="text-sm text-muted-foreground">{discussion.subject || 'General'} - {discussion.likes || 0} likes</p>
                      </div>
                      <Button
                        size="sm"
                        variant="destructive"
              ...
              {loadingSolutions ? (
                ...
                <div className="space-y-3">
                  {solutions.map((solution: any) => (
                    <div key={solution.id} className="flex items-center justify-between p-4 bg-muted/30 dark:bg-muted/10 rounded-lg" data-testid={`row-solution-${solution.id}`}>
                      <div>
                        <p className="font-medium text-foreground">{solution.title}</p>
                        <p className="text-sm text-muted-foreground">{solution.subject} - {solution.solutionType} - {solution.helpfulVotes || 0} votes</p>
                      </div>
                      <div className="flex items-center gap-2">
              ...
              {loadingAnnouncements ? (
                ...
                <div className="space-y-3">
                  {announcements.map((announcement: any) => (
                    <div key={announcement.id} className="flex items-center justify-between p-4 bg-muted/30 dark:bg-muted/10 rounded-lg" data-testid={`row-announcement-${announcement.id}`}>
                      <div className="flex-1">
                        <p className="font-medium text-foreground">{announcement.title}</p>
                        <p className="text-sm text-muted-foreground truncate max-w-md">{announcement.content}</p>
                      </div>
                      <div className="flex items-center gap-2">

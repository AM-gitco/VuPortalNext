...
export function Navbar({ user, onPageChange, onMobileMenuClick }: NavbarProps) {
  ...
  const logoutMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      // Redirect handled by action, but just in case:
      ...
      <div className="flex items-center gap-3">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" className="flex items-center gap-3 px-3 py-6 hover:bg-accent/50 rounded-2xl transition-all border border-border/20 hover:border-border shadow-sm hover:shadow-md group" data-testid="user-profile-dropdown">
              <div className="relative">
                <div className="w-10 h-10 bg-gradient-to-br from-primary to-primary-foreground rounded-full flex items-center justify-center shadow-lg text-white font-bold text-sm ring-2 ring-background group-hover:scale-105 transition-transform">
                  {getInitials(user?.fullName)}
                </div>
                <div className="absolute -bottom-1 -right-1">
                  <Badge variant={user?.role === 'admin' ? "default" : "secondary"} className="text-[10px] px-1.5 py-0 h-4 border-2 border-background shadow-lg">
              ...
              <div className="text-left hidden md:block space-y-0.5">
                <p className="text-sm font-bold text-foreground leading-tight group-hover:text-primary transition-colors" data-testid="text-username">
                  {user?.fullName}
                </p>
                <div className="flex items-center gap-2">
                  <Badge variant="outline" className="text-[10px] font-semibold h-4 px-1 bg-muted/50 border-none">
                    {user?.role === 'admin' ? 'ADMIN' : 'STUDENT'}
                  </Badge>
                  <span className="text-[10px] text-muted-foreground/80 font-bold uppercase tracking-wider">#{user?.id?.slice(0, 4)}</span>
                </div>
              </div>
            </Button>
          </DropdownMenuTrigger>

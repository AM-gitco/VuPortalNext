...
import type { Upload as UploadType } from "@/lib/db/schema";
...
export function UploadArea({ user }: { user: any }) {
  ...
  const { data: myUploads = [], isLoading } = useQuery<UploadType[]>({
    ...
    queryFn: queryFunctions.myUploads,
  });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-foreground" data-testid="text-upload-title">Upload Area</h1>
                ...
                >
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1">
                      <h4 className="font-semibold text-lg text-foreground mb-2">{upload.title}</h4>
